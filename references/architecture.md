...
> 2026-06-26 · Phase 2 完成 · Phase 3 已部分实现 · Phase 4 已实现
...
| **L3: Loop Adapter** | `hermes_adapter {gate,emit-tasks,compact}` / `langgraph_adapter` | Ralph、LangGraph、CrewAI 等编排器 | 薄适配器封装 |
...
gc_tick (FACADE —— 一个命令封装全部复杂度)
  ...
  │     首次 tick（无 findings.json）时跑完整 scan 替代 --dirty-only
  │     (OBSERVER —— 只读，永不编辑；无 dirty card 时保留上次 findings)
  │
...
### Phase 3 — Loop 接缝（已部分实现）
- `hermes_adapter gate` → 作为 loop 的 verify_cmd ✅
- `hermes_adapter compact` → 治 loop 自身 context 腐烂 ✅
- `hermes_adapter emit-tasks` → 升级项写成编排器任务 ✅
- `langgraph_adapter` → LangGraph 变体 ✅

### Phase 4 — MCP Server（已实现）
- `scripts/mcp_server.py` — stdio MCP server，暴露 tick/review_queue/resolve/profile ✅
...
│   ├── review_queue.py         # 聚合待决项 → review-queue.json（含 Mark→Sweep 桥）
│   ├── resolve.py              # Agent 自决 + 审计日志
│   ├── gc_tick.py              # 一次治理 tick（loop/agent 入口）
│   ├── collect.py              # 回收站 Sweep：move 不 delete，可 restore
│   ├── analyze_patterns.py     # L4 爬山法：从累积模式生成优化提案
...
│   └── adapters/               # Loop 适配器
│       ├── hermes_adapter.py   # gate / emit-tasks / compact
│       └── langgraph_adapter.py # LangGraph 变体
│
├── examples/                   # 10 个 demo
...
│   └── demo-hill-climb/        # L4 爬山法：累积模式 → 优化提案
│
├── references/                 # 参考文档
│   ├── gc-model.md             # GC ↔ 熵 思维模型
│   ├── entropy-checklist.md    # 垃圾分类学
│   ├── treatment-playbook.md   # 按类型清理动作（含 Collect 可逆 Sweep）
│   ├── hooks.md                # Hook 方案
│   ├── mcp-surface.md          # MCP 工具接口设计
│   ├── loop-engineering.md     # Loop 引擎工程参考
│   └── architecture.md         # 本文件
│
├── evals/evals.json            # 56 个机器可读 eval 场景
├── research/                   # 设计笔记
│   ├── context-gc-research.md  # 设计来源
│   ├── loop-integration-plan.md # Loop 集成开发计划
│   ├── skillopt-integration.md # SKILL 优化集成
...
python scripts/run_evals.py               # 56 eval 全部通过
