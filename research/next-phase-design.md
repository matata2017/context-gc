...
第1步【设计一·最小可行】  scope.json + 分支切换失效 dirty
   └─ 解 git 分支污染，代码小，无依赖。先做。
第2步【设计一·学习数据标注】 patterns/decisions 补 scope 字段
   └─ 自进化的前提。
第3步【设计二·候选生成】  analyze_patterns 加 --emit-eval（只生成候选，不合）
   └─ 依赖第2步。带 samples 门控。
第4步【设计三·LangGraph 节点】 langgraph_adapter.py（薄适配，不引入 langgraph 依赖）
   └─ 独立，可并行。
第5步【设计二·全闭环】  候选→人审 staging→优化→门控合
   └─ 依赖第3步 + skillopt_optimize（已有）。
```

每步都要：samples 共识验证 + 数据门控 + dogfood 绿。**先 review 本文档，逐步确认再造，不要一次全做。**
